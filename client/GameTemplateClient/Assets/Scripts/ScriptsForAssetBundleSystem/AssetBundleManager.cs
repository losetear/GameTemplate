...
public class AssetBundleManager : MonoBehaviour
{
    ...
    {
//        if (m_LoadedAssetBundles.ContainsKey("Android"))
//        {
//            m_LoadedAssetBundles["Android"].m_AssetBundle.Unload(true);
//            m_LoadedAssetBundles.Remove("Android");
//        }
        m_AssetBundleManifest = null;
        foreach (var mLoadedAssetBundle in m_LoadedAssetBundles)
        {
            mLoadedAssetBundle.Value.m_AssetBundle.Unload(true);
        }
...
#if dev
        Debug.Log("LoadAssetBundle start " + assetBundleName + " at frame " + Time.frameCount);
#endif
    ...
    {
#if dev
        Debug.Log("RemapVariantName start " + assetBundleName + " at frame " + Time.frameCount);
#endif
...
#if dev
        Debug.Log("RemapVariantName end " + assetBundleName + " at frame " + Time.frameCount);
#endif
        if (bestFitIndex != -1)
	...
	void Update()
	{
		...
		foreach (var keyValue in m_DownloadingWWWs)
		{
...
#if dev
                Debug.LogError("LoadAssetBundleInternal error name:" + keyValue.Key + " url:" + download.url+ " error:" + download.error);
#endif
                continue;
			}
			...
			if(download.isDone)
			{
                Debug.Log("Downloading " + keyValue.Key + " is done at frame " + Time.frameCount + "time" + Time.time);

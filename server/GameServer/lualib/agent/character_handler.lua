...
local function create (name, race, class)
	...
	syslog.notice ("finish create role")
	return character
end

local NAME_ALREADY_EXIST = 1
function REQUEST.character_create (args)
	for k, v in pairs (args) do print (k, v) end
	local c = args.character or error ("invalid argument")

	local character = create (c.name, c.race, c.gameclass)
	local id = skynet.call (database, "lua", "character", "reserve", uuid.gen (), c.name)
	if not id then
		syslog.notice ("reserve error")
		return { character={}, errno = NAME_ALREADY_EXIST }
	end
	syslog.noticef ("REQUEST.character_create id %s", id)
	...
	syslog.noticef ("REQUEST.character_create json %s", json)
...
function REQUEST.character_pick (args)
	...
	syslog.noticef ("REQUEST.character_pick %s",character)
...
function handler.init (character)
	syslog.noticef ("charaHandler init id %s", character.id)
	...
	local class = character.general.gameclass

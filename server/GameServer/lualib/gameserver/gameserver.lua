...
function gameserver.start (gamed)
	...
	local function do_login (fd, msg, sz)
		...
		syslog.noticef ("gameserver do_login session:%s token:%s", args.session,args.token)
	...
	function handler.message (fd, msg, sz)
		syslog.notice ("gameserver handler.message ")
		...
		if queue then
			...
			if ok then
				syslog.noticef ("gameserver account %d login success", account)
	...
	function handler.command (cmd, ...)
		local f = CMD[cmd]
		syslog.noticef ("gameserver handler.command %s",cmd)
		if f then
			return f (...)
		else

...
function gateserver.start (handler)
	syslog.notice("gateserver.start")

	function CMD.open (source, conf)
		syslog.notice("CMD.open")
	...
	local function dispatch_msg (fd, msg, sz)
		...
		local agent = c.agent
		if agent then
			--syslog.noticef ("gateserver dispatch_msg agent", agent)
			skynet.redirect (agent, 0, "client", 0, msg, sz)
		else
			--syslog.noticef ("gateserver dispatch_msg handler")
	...
	skynet.register_protocol {
		...
		dispatch = function (_, _, q, type, ...)
			queue = q
			if type then
				syslog.noticef ("gateserver socket %s", type)
				return MSG[type] (...) 
			end
		end,
	}
	...
	skynet.start (function ()
		skynet.dispatch ("lua", function (_, address, cmd, ...)
			syslog.noticef ("gateserver lua %s", cmd)

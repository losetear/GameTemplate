...
local sync_handler = require "agent.sync_handler"
...
local function send_msg (fd, msg)
	...
	--syslog.warningf ("send_msg msg:%d", #msg) 
end
...
local function handle_request (name, args, response)
	syslog.debugf ("agent handle_request %s", name)
...
local function handle_response (id, args)
	syslog.noticef ("agent handle_response %s", args)
	local s = session[id]
	if not s then
...
skynet.register_protocol {
	...
	unpack = function (msg, sz)
		--syslog.debug ("agent unpack")
		return host:dispatch (msg, sz)
	end,
	dispatch = function (_, _, type, ...)
		syslog.debugf ("agent RESPONSE type:%s",type)
